...
func NewCmdHelloRPC() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "setKyc [address]",
		Short: "pass the address which you want to aprove for kyc",
		Long:  `Pass the address which you want to approve for kyc`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			err = cmd.Flags().Set(flags.FlagMsg, args[1])
			if err != nil {
				return err
			}
			clientCtx, err := client.GetClientTxContext(cmd)
			...
			msg := types.NewSetKycRPC(clientCtx.GetFromAddress(), args[1])
			if err != nil {
				return err
			}
