...
func (ak AccountKeeper) GetKyc(c context.Context, req *types.GetSingleKycRequest) (*types.GetSingleKycResponse, error) {
	ctx := sdk.UnwrapSDKContext(c)

	store := ctx.KVStore(ak.storeKey)
	kycStore := prefix.NewStore(store, types.AddressStoreKeyPrefix)
	res := kycStore.Get([]byte(req.Address))
	fmt.Println(res, string(res))
	return &types.GetSingleKycResponse{Status: true}, nil
}

func (ak AccountKeeper) GetAllKyc(c context.Context, req *types.GetKycRequest) (*types.GetKycResponse, error) {
	...
	pageRes, err := query.Paginate(kycStore, req.Pagination, func(key []byte, value []byte) error {
		var kyc types.Kyc
		if err := ak.cdc.Unmarshal(value, &kyc); err != nil {
			return err
		}
	...
	return &types.GetKycResponse{Kyc: kycs, Pagination: pageRes}, nil
}
